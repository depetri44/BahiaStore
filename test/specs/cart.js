...
test('[Success] Add subscripton product to cart', async t => {
    ...
    t.deepEqual(res.body.message, 'Compra actualizado con éxito');
});
...
test('[Success] Vaciar Compra', async t => {
    const res = await g.request
        .post('/product/emptycart')
        .expect(200);
    t.deepEqual(res.body.message, 'Compra eliminada con éxito');
});

test('[Success] Añadir producto al carrito', async t => {
    const res = await g.request
        .post('/product/addtocart')
        .send({
...
test('[Fail] Try remove an item which is not in the cart', async t => {
    const res = await g.request
        ...
        .expect(400);
    t.deepEqual(res.body.message, 'El Producto no está disponible por el momento in cart');
});
...
test('[Fail] Try add more than the variant stock', async t => {
    ...
    const emptyCart = await g.request
        .post('/product/emptycart')
        .expect(200);
    t.deepEqual(emptyCart.body.message, 'Compra vaciado con éxito');
...
test('[Success] Hold some stock add more stock which is less than total', async t => {
    ...
    t.deepEqual(addToCart2.body.message, 'Compra actualizado con éxito');
});
