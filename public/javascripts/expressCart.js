...
$(document).ready(function (){
    ...
    $(document).on('change', '.cart-product-quantity', function (e){ cartUpdate(e.target); });
    ...
    $(document).on('click', '#btn_search', function(e){
        ...
        if($('#frm_search').val().trim() === ''){
            showNotification('Por favor ingrese un valor de búsqueda', 'danger');
        }else{
        ...
        if(session.totalCartDiscount > 0){
            discountTotal = `
                <div class="text-right">
                    Descuento: <strong id="discount-amount">${result.currencySymbol}${discountTotalAmt}</strong>
                </div>`;
        }
