...
router.get('/admin/product/edit/:id', restrict, checkAccess, async (req, res) => {
    ...
    if(!product){
        ...
        if(req.apiAuthenticated){
            res.status(400).json({ message: 'El Producto no está disponible por el momento' });
            return;
        }
        req.session.message = 'El Producto no está disponible por el momento';
...
router.post('/admin/product/addvariant', restrict, checkAccess, async (req, res) => {
    ...
    try{
        ...
        res.status(200).json({ message: 'Talle Agregado Correctamente', product });
    }catch(ex){
        console.log('here?');
...
router.post('/admin/product/editvariant', restrict, checkAccess, async (req, res) => {
    ...
    try{
        ...
        res.status(200).json({ message: 'Guardado con éxito variant', variant: updatedVariant.value });
    }catch(ex){
...
router.post('/admin/product/update', restrict, checkAccess, async (req, res) => {
    ...
    try{
        ...
        indexProducts(req.app)
        .then(() => {
            res.status(200).json({ message: 'Guardado con éxito', product: productDoc });
        });
    }catch(ex){
