...
router.get('/admin/order/delete/:id', restrict, async(req, res) => {
    ...
    try{
        ...
        .then(() => {
            ...
            // redirect home
            req.session.message = 'Pedido Eliminado con éxito';
            req.session.messageType = 'success';
            res.redirect('/admin/orders');
        });
    }catch(ex){
        ...
        // redirect home
        req.session.message = 'Error al eliminar Pedido';
...
router.post('/admin/order/updateorder', restrict, checkAccess, async (req, res) => {
    ...
    try{
        ...
        return res.status(400).json({ message: 'Fallo la actualizacion del Pedido' });
    }
});
