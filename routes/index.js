...
router.get('/product/:id', async (req, res) => {
    ...
    if(!product){
        res.render('error', { title: 'Not found', message: 'El Producto no está disponible por el momento', helpers: req.handlebars.helpers, config });
        return;
    }
...
router.post('/product/updatecart', async (req, res, next) => {
    ...
    res.status(200).json({ message: 'Compra actualizado con éxito', totalCartItems: Object.keys(req.session.cart).length });
});
...
router.post('/product/removefromcart', async (req, res, next) => {
    ...
    if(!req.session.cart[req.body.cartId]){
        return res.status(400).json({ message: 'El Producto no está disponible por el momento in cart' });
    }
    ...
    if(!req.params.page){
        Promise.all([
            ...
            .then(async([results, menu]) => {
                ...
                res.render(`${config.themeViews}index`, {
                    title: `${config.cartTitle} - Venta Online`,
                    theme: config.theme,
                    results: results.data,
                    session: req.session,
                    message: clearSessionValue(req.session, 'message'),
